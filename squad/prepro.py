...
def get_args():
    ...
    source_dir = "G:\cs224d_files\\all_dataset\SQuAD"
    target_dir = "data/squad"
    # glove_dir = os.path.join(home, "data", "glove")
    glove_dir = "G:\cs224d_files\BiLSTM-master\data\glove"
...
def prepro_each(args, data_type, start_ratio=0.0, stop_ratio=1.0, out_name="default", in_path=None):
    ...
    for ai, article in enumerate(tqdm(source_data['data'][start_ai:stop_ai])):
        ...
        for pi, para in enumerate(article['paragraphs']):
            ...
            context = context.replace("``", '" ')
            xi = list(map(word_tokenize, sent_tokenize(context)))  #sent_tokenize按句子分割, word_tokenize-分词
            xi = [process_tokens(tokens) for tokens in xi]  # process tokens
            # given xi, add chars
            cxi = [[list(xijk) for xijk in xij] for xij in xi]  # 字符级别"abc"=>["a","b","c"]
            xp.append(xi)
            cxp.append(cxi)
            pp.append(context)
...
def test():
    source_dir = "G:\cs224d_files\\all_dataset\SQuAD"
    data_type = "dev"
    source_path = os.path.join(source_dir, "{}-v2.0.json".format(data_type))
    print(source_path)
    source_data = json.load(open(source_path, 'r'))
    ...
    print(len(source_data['data']), start_ai, stop_ai)
